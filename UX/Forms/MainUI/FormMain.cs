...
public partial class FormMain : Form
{
    ...
    {
        string txt = LearningAndRaceManager.s_generation.ToString();

        // show progress.
        if (LearningAndRaceManager.s_currentBestCarId > -1)
        {
            if (LearningAndRaceManager.s_gatesHalf1.Length > 0)
                txt += $" {LearningAndRaceManager.s_cars[LearningAndRaceManager.s_currentBestCarId].CurrentGate}/{LearningAndRaceManager.s_gatesHalf1.Length}";
        ...
        // we're not in quiet mode, so we show when mutate is next going to happen
        //labelMovesToNextMutate.Text = LearningAndRaceManager.s_movesLeftBeforeNextMutation.ToString();
    ...
    private void ConfigureVisionToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (LearningAndRaceManager.InLearning) RestartLearning();
    }
